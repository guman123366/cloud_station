...
ZhouShanProtocol::ZhouShanProtocol(QObject *parent)
	: ConstructionProtocolInterface(parent)
{
}

ZhouShanProtocol::~ZhouShanProtocol()
{
}

int ZhouShanProtocol::ConstructionData(unsigned char* sendBuf)
{
	if (!ReceiveUAVData)
		return 0;
	...
	RootJasonObject.insert("uavSN", "UAS03403379");
	...
	RootJasonObject.insert("rollData", ProtocolData->m_FirstSubThreeFrame.Roll);//滚转角;
	RootJasonObject.insert("pitchData", ProtocolData->m_FirstSubThreeFrame.Pitch);//俯仰角;
	RootJasonObject.insert("headingData", ProtocolData->m_FirstSubThreeFrame.Yaw);//偏航角;
...
//#include <QDateTime>
//QString ZhouShanProtocol::QDateTime::currentDateTime().toMSecsSinceEpoch()
//{
