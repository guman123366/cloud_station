...
IntelligentAirSpaceProtocol::IntelligentAirSpaceProtocol(QObject *parent)
	: ConstructionProtocolInterface(parent)
{
}

IntelligentAirSpaceProtocol::~IntelligentAirSpaceProtocol()
{
}

int IntelligentAirSpaceProtocol::ConstructionData(unsigned char* sendBuf)
{
	if (!ReceiveUAVData)
		return 0;

	TD220TelemetryData* ProtocolData = (TD220TelemetryData*)ReceiveUAVData;
	if (!ProtocolData)
		return 0;
	...
	DeviceDataObject.insert("deviceHardId", "2763H7M001N342");
	QJsonObject DataJasonObject;
	DataJasonObject.insert("openid", "13234");
	QJsonObject UavInfoObject;
	UavInfoObject.insert("armd", 1);
	UavInfoObject.insert("climbRate", -(ProtocolData->UAVVelocity.m_dDown));//爬升率
	QJsonObject CustomJasonObject;
	CustomJasonObject.insert("maxHeight", 3500);	 
	UavInfoObject.insert("customData", QJsonValue(CustomJasonObject));//最大飞行高度
	UavInfoObject.insert("latitude", ProtocolData->UAVAbsPosition.m_dLat*ZHZ_GLOBAL_dRadianToAngle);//纬度
	UavInfoObject.insert("height", -ProtocolData->UAVRelPosition.m_dDown);//高程/相对高度
	UavInfoObject.insert("taskId", 53278);//任务ID
	ValueTemp = ProtocolData->UAVVelocity.m_dNorth*cos(ProtocolData->UAVAttitude.yaw)
		+ ProtocolData->UAVVelocity.m_dEast*sin(ProtocolData->UAVAttitude.yaw);
	UavInfoObject.insert("airSpeed", ValueTemp);//飞行速度
	ValueTemp = abs(sqrt(pow(ProtocolData->UAVVelocity.m_dNorth, 2) + pow(ProtocolData->UAVVelocity.m_dEast, 2)));
	UavInfoObject.insert("groundSpeed", ValueTemp);//地速
	ValueTemp = abs(sqrt(pow(ProtocolData->UAVRelPosition.m_dNorth, 2) + pow(ProtocolData->UAVRelPosition.m_dEast, 2)));
	UavInfoObject.insert("distanceToHome", ValueTemp);//距离Home点距离
	UavInfoObject.insert("distanceToNext", ProtocolData->WayPointInfo.m_dDist);//到下一航点距离
	UavInfoObject.insert("uid", 0);//用户ID
	ValueTemp = ProtocolData->UAVAttitude.yaw* ZHZ_GLOBAL_dRadianToAngle;
	ValueTemp = ValueTemp >= 0 ? ValueTemp : 360 + ValueTemp;
	UavInfoObject.insert("yaw", ValueTemp);//方位角
	ValueTemp = ProtocolData->UAVAttitude.roll* ZHZ_GLOBAL_dRadianToAngle;
	UavInfoObject.insert("roll", ValueTemp);//滚转角
	ValueTemp = ProtocolData->UAVAttitude.pitch* ZHZ_GLOBAL_dRadianToAngle;
	UavInfoObject.insert("pitch", ValueTemp);//倾斜角
	UavInfoObject.insert("longitude", ProtocolData->UAVAbsPosition.m_dLon*ZHZ_GLOBAL_dRadianToAngle);//经度
	UavInfoObject.insert("platformType", 0);//平台类型
	UavInfoObject.insert("voltage", ProtocolData->UAVAircraft.m_dBatFCS);//电池电压
	UavInfoObject.insert("flightDistance", 0);//飞行里程
	UavInfoObject.insert("barometerTemp", -(ProtocolData->UAVVelocity.m_dDown));//气压计温度
	UavInfoObject.insert("isLocation", ProtocolData->UAVAircraft.m_SateMode);//是否定位成功
	UavInfoObject.insert("current", 0);//电流
	UavInfoObject.insert("flightState", 1);//飞行状态
	DataTemp = QString::number(-(ProtocolData->UAVVelocity.m_dDown), 'f', 2);
	UavInfoObject.insert("unmannedId", 1);//无人机类型
	UavInfoObject.insert("battaryRemain", ProtocolData->UAVADValue.m_dLastOil);//剩余油量
	...
	UavInfoObject.insert("altitude", ProtocolData->UAVAbsPosition.m_dAlt);//高程/海拔高度
	UavInfoObject.insert("flightSortie", "zhz20230829");//架次号
	DataTemp = QString::number(ProtocolData->UAVADValue.m_dLastOil, 'f', 2);
	UavInfoObject.insert("flightMode", "速度模式");//飞行模式
	DataTemp = QString::number(ProtocolData->UAVADValue.m_dLastOil, 'f', 2);
	UavInfoObject.insert("flightTime", 100);//飞行时长
	UavInfoObject.insert("satCount", ProtocolData->UAVAircraft.m_nSat);//卫星数
